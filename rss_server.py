...
def generate_daily_content(lng, lat, city_name):
    """获取天气 + 匹配诗词，返回内容字典"""
    weather = fetch_weather(CONFIG["caiyun_token"], lng, lat)

    # 使用日期+坐标作为随机种子，保证同一天同一位置选到同一首诗
    today = date.today().isoformat()
    seed_str = f"{today}_{lng}_{lat}"
    seed = int(hashlib.md5(seed_str.encode()).hexdigest()[:8], 16)
    rng = random.Random(seed)

    poem, matched_trigger = select_poem(POETRY_DB, weather["triggers"])
    if not poem:
...
def get_cached_content(lng, lat, city_name):
    """获取缓存内容（同一天同一位置只生成一次）"""
    cache_key = f"{lng:.4f},{lat:.4f}"
    today = date.today().isoformat()
    ...
    # 生成新内容
    try:
        data = generate_daily_content(lng, lat, city_name)
...
def generate_rss_xml(content):
    ...
    today = date.today().isoformat()
    guid = f"poetry-weather-{today}-{city}"
...
def rss():
    ...
    content = get_cached_content(lng, lat, city_name)
...
def main():
    ...
    print(f"  RSS 订阅地址: http://0.0.0.0:{port}/rss")
